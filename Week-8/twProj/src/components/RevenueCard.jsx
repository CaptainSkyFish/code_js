export const RevenueCard = ({ title, amount, orderCount }) => {
  return (
    <div
      className={`bg-white shadow-md rounded-lg m-5 flex-grow p-5 min-w-[300px] h-fit`}
    >
      <div className="flex flex-col p-5 gap-4">
        <h5 className="flex items-center graytext">
          <div>{title}</div>
          <svg
        ...
        <div className="flex justify-between items-center">
          ...
          {!orderCount ? null : (
            <a
              className={`text-sm underline text-revCardBlue font-medium`}
